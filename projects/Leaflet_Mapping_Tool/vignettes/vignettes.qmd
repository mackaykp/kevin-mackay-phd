...
subtitle: "Income × green area per 1,000 people by place and scale"
sidebar: false
body-class: portfolio-browse-page
format:
  html:
...
<div class="portfolio-browse-page-root" hidden aria-hidden="true"></div>

<style>
/* Browse-style page tokens (aligned with site palette; page chrome uses global beige) */
:root {
  --br-bg: transparent;
  --br-surface: #ffffff;
  --br-border: rgba(1, 38, 38, 0.15);
  --br-text: #000000;
  --br-text-muted: rgba(1, 38, 38, 0.62);
  --br-heading: #012626;
  --br-accent: #bf532c;
  --br-radius: 10px;
  --br-radius-sm: 6px;
  --br-shadow: 0 1px 2px rgba(1, 38, 38, 0.06);
}
...
.quarto-title-block.default {
  ...
  border-left: 4px solid var(--br-accent) !important;
  border-radius: 0.85rem !important;
  box-shadow: 0 2px 10px rgba(1, 38, 38, 0.06);
}
.quarto-content h2 { font-size: 1.35rem !important; font-weight: 700 !important; color: var(--br-heading) !important; margin-top: 2rem !important; }
...
These vignettes mirror how the Leaflet mapping tool supports real research questions: you can run the bivariate view shown here (income × green area per 1,000 people at the dissemination area level) or adapt the same pipeline to other variables and geographies. The examples walk through different use cases to showcase location options (e.g., cities, custom bounding box, etc.), buffer and layer options, and conceptually beyond Canada when you supply equivalent boundaries and attributes.

The maps shown here are pre-generated and embedded so the page stays fast and so we are not repeatedly querying OpenStreetMap from every visitor’s browser. When you run the tool locally, repeated runs for the same city or bbox benefit from a caching database that stores OSM downloads and avoids abusing Overpass. The code under each map is the exact call used to create it. 
...
All maps on this page were produced with the same pipeline; only these parameters change. If you batch many maps in a short window without using the cache, OpenStreetMap’s services can rate-limit requests; the project’s caching layer is there so repeat or incremental work reuses downloaded data and stays within normal use. Under typical single-map or cache-backed runs, you should not see that problem.
