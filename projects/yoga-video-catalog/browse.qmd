...
<style>
/* Design tokens - browse (aligned with site palette; page chrome uses global beige) */
:root {
  --br-bg: transparent;
  --br-surface: #ffffff;
  ...
  --br-heading: #012626;
  --br-accent: #bf532c;
  --br-accent-soft: rgba(191, 83, 44, 0.1);
  --br-radius: 10px;
  --br-radius-sm: 6px;
  --br-shadow: 0 1px 2px rgba(1, 38, 38, 0.06);
  --br-shadow-md: 0 4px 12px rgba(1, 38, 38, 0.08);
  --br-chip: rgba(243, 234, 231, 0.9);
  --br-chip-active: #bf532c;
  --br-chip-active-bg: rgba(191, 83, 44, 0.12);
}
...
.browse-chip:hover { background: rgba(1, 38, 38, 0.08); }
...
.browse-duration-slider input[type="range"]::-webkit-slider-thumb:hover {
  transform: scale(1.1);
  box-shadow: 0 3px 10px rgba(191, 83, 44, 0.28);
}
...
csv_path <- "data/yoga_videos_enriched.csv"
if (!file.exists(csv_path)) csv_path <- file.path("projects", "yoga-video-catalog", "data", "yoga_videos_enriched.csv")
if (!file.exists(csv_path)) csv_path <- file.path("..", "data", "yoga_videos_enriched.csv")
if (file.exists(csv_path)) {
  dat <- read.csv(csv_path, stringsAsFactors = FALSE, na.strings = c("", "NA"))
} else {
  dat <- data.frame(
    video_id = character(), title = character(), duration = character(), view_count = character(),
    upload_date = character(), focus = character(), channel = character(), url = character(),
    duration_seconds = numeric(), stringsAsFactors = FALSE
  )
}
